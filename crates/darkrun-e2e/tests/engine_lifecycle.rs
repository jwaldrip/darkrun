...
#[test]
fn drift_preempts_run_track() {
    use darkrun_core::domain::{Drift, DriftKind};
    let h = Harness::start("dr");
    ...
    .unwrap();
    assert_eq!(h.position().track, Track::Drift);
}

#[test]
fn drift_preempts_even_open_feedback() {
    use darkrun_core::domain::{Drift, DriftKind};
    let h = Harness::start("dr");
    h.file_feedback("fb-1", "pending", "feedback present");
    darkrun_mcp::drift::record(
        &h.store,
        "dr",
        "d-01",
...
fn drift_action_carries_path() {
    ...
    .unwrap();
    match h.position().action {
        Some(RunAction::ResolveDrift { path, .. }) => assert_eq!(path, "frame/output.md"),
        other => panic!("expected ResolveDrift, got {other:?}"),
    }
}

#[test]
fn drift_action_serializes_resolve_drift_tag() {
    use darkrun_core::domain::{Drift, DriftKind};
    let h = Harness::start("dr");
    darkrun_mcp::drift::record(
        &h.store,
    ...
    .unwrap();
    let a = h.position().action.unwrap();
    assert_eq!(action_tag(&a), "resolve_drift");
}
